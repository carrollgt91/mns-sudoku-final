...
public class Sudoku {
	...
	public static boolean validate(int[][] cells) {
		Set<Integer> completeUnit = new TreeSet<Integer>();
		...
		for(int i = 0; i < 9; i++)
		{
			Set<Integer> rowContents = new TreeSet<Integer>();

			for(int j = 0; j < 9; j++)
			{
				rowContents.add(cells[i][j]);
			}

			if(!rowContents.equals(completeUnit))
				return false;
		...
		for(int i = 0; i < 9; i++)
		{
			Set<Integer> colContents = new TreeSet<Integer>();

			for(int j = 0; j < 9; j++)
			{
				colContents.add(cells[i][j]);
			}

			if(!colContents.equals(completeUnit))
				return false;
		}
		
		for(int i = 0; i < 9; i++)
		{
			Set<Integer> boxContents = new TreeSet<Integer>();

			for(int j = 0; j < 9; j++)
			{
				int row = 3*(i/3) + (j/3);
				int col = 3*(i%3) + (j%3);
				boxContents.add(cells[row][col]);
			}

			if(!boxContents.equals(completeUnit))
				return false;
		}
